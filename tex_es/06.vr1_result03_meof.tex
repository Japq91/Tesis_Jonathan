...
El contraste con la Población Global es inmediato y profundo. El campo de precipitación (paneles a, d, g) pierde por completo la extensión difusa del patrón global y desarrolla una estructura multilobular. En SBR (panel a) aparece un núcleo de anomalías positivas intensas y compactas ($>+0.013$) próximo al vórtice, ligeramente desplazado al este y al sur ($\Delta x \approx +1^{\circ}$ a $+3^{\circ}$, $\Delta y \approx -0.5^{\circ}$ a $-2^{\circ}$), flanqueado por dos lóbulos de anomalías negativas, uno al noreste y otro al sur, configurando ya una estructura asimétrica de lóbulos diferenciados.
En LPB (panel d), el patrón de precipitación muestra una banda de anomalías positivas con inclinación noroeste-sureste, con el núcleo más intenso próximo al vórtice y una extensión secundaria hacia el noroeste, flanqueada por anomalías negativas concentradas al noreste y, en menor medida, al sur.
En ARG (panel g), el núcleo positivo de precipitación es el más intenso y compacto de los tres, centrado sobre el vórtice y extendido hacia el este ($\Delta x \approx 0^{\circ}$ a $+5^{\circ}$, $\Delta y \approx -2^{\circ}$ a $+1^{\circ}$), con una carga que alcanza $+0.018$ en el máximo, y con anomalías negativas débiles al oeste y al noreste.
El campo de viento (paneles b, e, h) exhibe un comportamiento distinto. En las tres regiones, las anomalías son negativas en la mayor parte del dominio, más intensas hacia el norte, con una franja de valores neutros o levemente positivos hacia el sur del vórtice, donde en LPB y ARG llega incluso a tornarse débilmente positiva.
Este patrón, viento que se intensifica negativamente hacia el norte mientras la precipitación se concentra cerca del vórtice y hacia el sur, revela ya el germen de la separación de cuadrantes. Los máximos de precipitación y viento no covarían positivamente, sino que el MEOF1 captura una configuración de anticovarianza incipiente, con el forzamiento pluviométrico concentrándose donde el forzamiento eólico es más débil.
...
Esta separación prematura es la razón por la que el MEOF1 ya captura una configuración de anticovarianza en esta fase en los sistemas intensos. La precipitación se concentra cerca del vórtice y hacia el sur mientras el viento se intensifica hacia el norte, anticipando la configuración opuesta de cuadrantes que dominará en la madurez.
...
La característica más notable es que los patrones de precipitación (paneles a, d, g) y viento (paneles b, e, h) exhiben anomalías estandarizadas de signo positivo extensas y de gran escala, pero orientadas en ejes distintos del dominio. En las tres regiones, la precipitación presenta cargas positivas en la mitad oeste (más intensas hacia el suroeste) y negativas en la mitad este, mientras el viento presenta cargas positivas en la mitad norte, decreciendo gradualmente hacia el sur sin llegar a cambiar de signo.
Esta configuración, aunque no constituye una co-localización estricta, refleja el acoplamiento simple y predecible que caracteriza al ciclón ordinario maduro: ambos campos varían suavemente en todo el dominio, sin desarrollar núcleos aislados ni transiciones abruptas, aunque sus gradientes dominantes se orientan en ejes distintos (oeste-este para la precipitación, norte-sur para el viento), reflejo de que, en el régimen medio, cada campo responde al mismo forzante sinóptico de gran escala sin requerir una geometría idéntica.
La especificidad regional reside en la magnitud y forma de las cargas: LPB (paneles d--f) muestra la mayor varianza explicada (26.59\%) y las cargas de viento más intensas y uniformes, con valores positivos que cubren prácticamente toda la mitad norte del dominio (panel e); SBR (paneles a--c) exhibe un patrón de precipitación con cargas positivas que cubren la mitad oeste del dominio, más intensas hacia el suroeste (panel a); ARG (paneles g--i) muestra la menor varianza explicada (22.31\%) y patrones más difusos, reflejando la mayor variabilidad estructural de los sistemas de esta región.
...
El campo de viento (paneles b, e, h) presenta una configuración opuesta. Las anomalías positivas se concentran en un núcleo compacto al oeste del centro, aproximadamente a su misma latitud, con cargas que alcanzan $+0.013$ en SBR (panel b) y $+0.010$ en LPB (panel e).
Este núcleo de viento al oeste y el arco de precipitación al sureste no sólo no comparten sector, sino que en el dominio lagrangiano se ubican en lados opuestos respecto al centro del vórtice (0,0). Las series de PC1 (paneles c, f, i), con solo 60--65 ciclones por región, muestran pulsos de amplitud extrema que superan con frecuencia el rango $\pm 100$, evidenciando que cada evento individual del subconjunto p90 proyecta una huella muy pronunciada sobre el modo dominante, a diferencia del comportamiento distribuido de la Población Global.
...
Se evidencia la reorganización espacial entre la banda de precipitación periférica en el cuadrante sureste y el núcleo de viento al oeste del vórtice, constituyendo la firma diagnóstica de la estructura ocluida en los sistemas p90 del Atlántico Sur.}
    \label{fig:meof_madurez_p90}
\end{figure}

La configuración de sectores opuestos documentada en el MEOF1 de madurez del subconjunto p90 es la evidencia multivariada que integra y confirma todos los resultados de los capítulos precedentes.
...
El núcleo de viento al oeste del MEOF1 coincide parcialmente con la concentración espacial identificada en el PDFe de viento p90 (Sección~\ref{sec:kde_espacial_p90}) y con la estructura compacta del EOF1 univariado de viento (Sección~\ref{subsec:eof1_wind10_p90}); aunque ambos coinciden en el sector occidental, el núcleo univariado se hiperconcentra específicamente en el cuadrante noroeste ($\Delta y \approx +2^{\circ}$ a $+3^{\circ}$), mientras que en el MEOF1 el núcleo se ubica prácticamente a la misma latitud del vórtice ($\Delta y \approx 0^{\circ}$), un desplazamiento hacia el sur que refleja la restricción adicional de covariar con la banda de precipitación. La novedad que aporta el MEOF es que, al descomponer la covarianza conjunta, demuestra que la separación entre ambos campos no es la yuxtaposición accidental de dos patrones independientes, sino una configuración geométrica estructuralmente coherente. El MEOF1 la captura como un único modo multivariado, lo que confirma que la precipitación al sureste y el viento al oeste covarían en los mismos eventos, constituyendo el patrón acoplado dominante de la estructura en el Atlántico Sur.

La interpretación física de esta reorganización espacial es consistente con el modelo de ciclón ocluido de Shapiro-Keyser \citep{schultz2021antecedents}. La intensificación acelerada del sistema se acompaña de aire frío y subsidente que desciende al oeste de su centro, suprimiendo la convección en el núcleo y confinando la humedad residual del WCB a la periferia exterior, consistente con la subsidencia troposférica documentada para el Hemisferio Norte por \citet{shapiro1999bridge}. La separación documentada valida el marco teórico de la reorganización dinámica del sistema durante la oclusión, donde la cinta transportadora fría (CCB) se consolida en el sector occidental mientras el WCB residual se reconfigura en la banda periférica sureste.
...
En la Población Global, las correlaciones entre $\text{PC1}^{\text{precip}}$ y $\text{PC1}^{\text{viento}}$ son positivas y significativas durante las fases iniciales en las tres regiones (Tabla~\ref{tab:meof_coherencia}), con valores que oscilan entre $+0.18$ y $+0.47$. En SBR y LPB, esta correlación se vuelve negativa, aunque débil, y significativa en la madurez y el decaimiento ($r=-0.19$ y $r=-0.08$, respectivamente), coherente con el inicio de la diferenciación entre las trayectorias de ambos campos para los sistemas que comienzan a ocluirse. En ARG, en cambio, el acoplamiento positivo se mantiene significativo durante todo el ciclo de vida, fortaleciéndose hacia la madurez ($r = +0.44$) antes de atenuarse en el decaimiento ($r = +0.23$), consistente con las oclusiones más graduales y el mayor control orográfico ya documentados en los capítulos previos (Secciones~\ref{sec:pdf_wind10} y~\ref{sec:pdf_tp}).

La incoherencia temporal significativa durante la madurez para los sistemas p90, presente en las tres regiones ($r = -0.45$ en SBR, $r = -0.37$ en LPB y $r = -0.31$ en ARG), constituye la evidencia estadística definitiva del \textit{desacoplamiento dinámico}. Un valor de correlación negativo entre las PC1 univariadas de precipitación y viento indica que, dentro del subconjunto de sistemas de alta vorticidad, los ciclones con la proyección más positiva sobre el modo de precipitación (banda enroscada intensa en el sureste) tienden a tener la proyección menos positiva, o negativa, sobre el modo de viento (núcleo intenso al oeste), y viceversa. Esto no implica que las estructuras no coexistan en el mismo ciclón, sino que su organización espacial interna es tal que cuando la banda de precipitación ocupa el cuadrante sureste, el campo de viento se ha reconfigurado hacia el sector occidental, y la estructura dominante de variabilidad de cada campo opera en sectores distintos del dominio. Estos resultados son consistentes con la reorganización dinámica de la oclusión ya descrita (Sección~\ref{subsec:meof_madurez}). A medida que los ciclones se intensifican, la intrusión seca desacopla la fuente de humedad de la circulación de bajo nivel, maximizando la separación espacial entre los campos de viento y precipitación durante la madurez.
...
En los ciclones de la Población Global, el MEOF1 captura un patrón de covarianza simple y extenso donde viento y precipitación permanecen mayormente positivos en todo el dominio lagrangiano, sin núcleos aislados ni cambios de signo abruptos, con una varianza conjunta explicada de entre 19\% y 27\%. Esta configuración refleja el desarrollo baroclínico clásico del modelo noruego de frentes \citep{bjerknes1922life}, sistematizado posteriormente en el marco del WCB como estructura termodinámica única que organiza el ascenso húmedo (precipitación) y el flujo de bajo nivel (viento) en el sector cálido del ciclón \citep{browning1986conceptual}.
La suavidad y extensión compartida de las cargas MEOF1 de ambas variables, aunque orientadas en ejes distintos (oeste-este en precipitación, norte-sur en viento), es la expresión cuantitativa de que, en el régimen medio, los dos campos son manifestaciones del mismo forzante sinóptico y operan como una estructura dinámica acoplada, sin que ello implique una co-localización geométrica estricta.
...
En el Hemisferio Sur, el MEOF proporciona evidencia equivalente pero en el espacio de la covarianza espacial conjunta. El modo dominante ya no captura un patrón simple y extenso, sino la geometría de separación entre los dos campos dinámicos.
...
La organización del viento en el sector occidental es consistente con la caracterización de la CCB como principal motor de la circulación de bajo nivel en la fase de oclusión, que \citet{martinez2014cold} documenta observacionalmente y que \citet{eisenstein2023identification} confirma en climatologías europeas.
...
La contribución del MEOF en el presente trabajo es demostrar que estas dos estructuras, documentadas separadamente en la literatura, covarían sistemáticamente en los ciclones p90 del Atlántico Sur. La presencia de la banda enroscada sureste va acompañada de la presencia del núcleo de viento al oeste dentro del mismo evento, constituyendo el prototipo de la estructura ocluida en la región.
...
Primero, en términos de la dimensionalidad del acoplamiento, mientras que en la Población Global el MEOF1 captura ~20--27\% de la covarianza conjunta con un patrón simple donde ambas variables permanecen positivas y varían suavemente en ejes distintos del dominio (oeste-este en precipitación, norte-sur en viento), en los sistemas de alta vorticidad (p90) la varianza cae a 12--19\% y se distribuye entre múltiples modos competitivos, expresando la fragmentación de mesoescala que opera cuando la intensificación del vórtice es mayor.
Segundo, en términos de la geometría del acoplamiento, el MEOF1 de madurez del subconjunto p90 captura un patrón de anticovarianza donde la precipitación y el viento se organizan en sectores opuestos del dominio lagrangiano, con el arco de precipitación enroscada al sureste y el núcleo de viento al oeste, separados por la intrusión seca.
