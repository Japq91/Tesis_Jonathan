...
%
%  La metodología se organiza como un pipeline secuencial
%  de cinco etapas, donde cada etapa produce el insumo
%  de la siguiente:
%
%  [M1] DATOS
%       ERA5 (1979-2020) + base de trayectorias con fases
%       ↓
%  [M2] POBLACIONES
%       Población Global (ciclo completo) + subconjunto p90
%       → estratificados por región (SBR / LPB / ARG)
%       ↓
%  [M3] PROCESAMIENTO LAGRANGIANO
%       Extracción de campos horarios centrados (20°×20°)
%       + promediado del núcleo temporal de cada fase
%       → un campo promedio por ciclón–fase
%       ↓
%  [M4] CARACTERIZACIÓN ESTADÍSTICA UNIVARIADA
%       Extracción de máximos → PDF(magnitud) + PDFe(ubicación)
%       EOF univariado → PC1 → Bootstrap-t
%       ↓
%  [M5] ANÁLISIS MULTIVARIADO Y PROTOTIPOS
%       MEOF(precipitación + viento) → composito condicionado
%       → superposición PDFe + loadings → figura 3×4
% ============================================================

% ============================================================
%  SECCIÓN M1 — DATOS
%  Propósito: Describir las dos fuentes de datos que alimentan
%  el pipeline: el reanálisis ERA5 (campos atmosféricos) y la
%  base de trayectorias con fases (CycloPhaser). Incluye
%  variables seleccionadas, período y limitaciones conocidas.
% ============================================================

\section{Diseño general}\label{sec:diseno_general}

% --- Síntesis del objetivo y la lógica del pipeline ---
...
% --- Descripción del producto y justificación de su elección ---
Para la caracterización de los campos atmosféricos se utilizó el reanálisis de quinta generación del ECMWF, ERA5, descrito por \citet{hersbach2020era5}. Se analizó el período comprendido entre 1979 y 2020, accediendo específicamente a las variables de precipitación y viento a través del conjunto de datos \textit{``ERA5 hourly data on single levels from 1940 to present''} \citep{hersbach_era5_2023b}. La elección de ERA5 resulta crítica para este diseño experimental: su resolución espacial ($\sim$31 km) y temporal (horaria) permite resolver los gradientes de presión y los flujos de humedad que generaciones anteriores de reanálisis suavizaban, superando las limitaciones documentadas por \citet{hodges2011comparison} para el Hemisferio Sur. Validaciones específicas para el Atlántico Sur realizadas por \citet{gramcianinov2020analysis} confirman que ERA5 ofrece una representación superior de la intensidad de los vientos superficiales en comparación con productos previos.

% --- Limitación documentada en extremos ---
No obstante, debe reconocerse una limitación documentada en la representación de extremos: \citet{chen2024evaluation} demuestran que ERA5 subestima sistemáticamente los extremos del 5\% de ciclones extratropicales más intensos, con sesgos del $-10.2\%$ en viento y $-22.6\%$ en precipitación respecto a observaciones satelitales y estaciones superficiales. Esta tendencia implica que los resultados obtenidos para el subconjunto p90 —correspondiente a los sistemas más intensos— deben interpretarse como estimaciones conservadoras de la severidad real de los eventos.

\subsection{Variables meteorológicas}\label{subsec:variables}

% --- Selección y justificación de las variables ---
...
El análisis conjunto de ambas variables responde al enfoque de eventos compuestos descrito en la Sección~\ref{subsec:tb_compound_def} del Marco Teórico, y fundamenta la posterior aplicación del análisis multivariado (MEOF) detallado en la Sección~\ref{subsec:eof_multivariado}.

\subsection{Base de trayectorias y fases}\label{sec:bd_ciclones}

% --- Descripción de la base de ciclones enriquecida ---
Para la identificación y caracterización de los sistemas, esta investigación utiliza la base de datos procesada por \citet{coutodesouza2024new}, quien tomó el catálogo original de trayectorias desarrollado por \citet{gramcianinov2019properties} y le incorporó una clasificación objetiva de las fases evolutivas mediante el algoritmo \textit{CycloPhaser} \citep{deSouza2025CycloPhaser}. El insumo principal de este trabajo combina así un seguimiento espacial robusto con un diagnóstico dinámico a lo largo de todo el ciclo de vida del ciclón. Los fundamentos que justifican el uso de la vorticidad relativa y la segmentación del ciclo de vida fueron expuestos en las Secciones~\ref{sec:tb_vorticidad} y~\ref{subsec:tb_fases} del Marco Teórico; esta sección se limita a describir las especificaciones técnicas y los criterios de filtrado del conjunto de datos.

% --- Especificaciones técnicas del tracking ---
...
% --- CycloPhaser como capa de enriquecimiento ---
...
% --- Qué se compara y qué preguntas responde la comparación ---
...
%  Propósito: Describir cómo se extraen y promedian los campos
%  horarios de ERA5 en coordenadas relativas al centro de cada
%  ciclón, produciendo un único campo promedio por ciclón–fase
%  que alimenta todos los análisis estadísticos posteriores.
% ============================================================

\section{Procesamiento lagrangiano de los campos de superficie}\label{sec:procesamiento_lagrangiano}

% --- Marco de referencia y dominio móvil ---
Adoptando el marco de referencia lagrangiano fundamentado en la Sección~\ref{subsec:tb_lagrangiano}, se define un dominio móvil de $20^{\circ} \times 20^{\circ}$ centrado en la posición del máximo de vorticidad para cada instante $t$ de la trayectoria, en coordenadas relativas $(\Delta x, \Delta y)$ respecto al núcleo ciclónico. Este dominio es suficientemente amplio para capturar los brazos frontales del sistema y suficientemente acotado para aislar la señal ciclónica de la variabilidad de gran escala de fondo.

% --- Estratificación por región y fase ---
...
%  Propósito: Describir las tres técnicas aplicadas de forma
%  independiente a cada variable (precipitación y viento):
%  (M4a) extracción de máximos y PDF de magnitud,
%  (M4b) PDFe bidimensional de localización espacial,
%  (M4c) EOF univariado con Bootstrap-t y análisis de PC1.
% ============================================================
...
donde $h$ es el ancho de banda (\textit{bandwidth}) y $K(\cdot)$ es el núcleo gaussiano:
...
El parámetro $h$ se determina automáticamente aplicando la regla de Scott. Como señala \citet{weglarczyk2018kernel}, este enfoque evita el sesgo inherente al agrupamiento en clases de un histograma, preservando la ubicación exacta de cada observación. Las curvas PDF resultantes permiten contrastar cuantitativamente cómo evolucionan las magnitudes de las variables a lo largo del ciclo de vida y cómo difieren entre la Población Global y el subconjunto p90.

\subsection{Distribución espacial de máximos: PDFe bidimensional}\label{subsec:kde_localizacion}

% --- Extensión 2D del KDE para la localización de máximos ---
Para caracterizar la distribución espacial preferencial de los máximos, se utiliza el conjunto de $m$ ubicaciones relativas $\{\mathbf{s}_i\}_{i=1}^{m}$ extraídas en la etapa anterior. Se extiende la formulación unidimensional de la Ecuación~\ref{eq:kde_pdf} a un estimador bidimensional (PDFe):
...
con núcleo gaussiano bidimensional:
...
% --- Niveles de contorno y coordenada modal ---
El resultado es un campo continuo de densidad de probabilidad relativa sobre el dominio centrado. Para su visualización se extraen dos niveles de contorno correspondientes a los percentiles 75 y 90 de la distribución acumulada de la densidad, calculados según:
...
% --- Anomalías y descomposición modal ---
Para identificar los patrones espaciales dominantes de variabilidad estructural, se aplica un análisis EOF sobre el conjunto de $m$ campos promedio por ciclón--fase, de forma separada para cada variable (precipitación, viento), región de ciclogénesis y fase evolutiva. Previo al análisis, a cada campo se le resta la media del ensamble en cada punto de grilla para obtener las anomalías estructurales:
...
Para evaluar la robustez estadística de los patrones espaciales (autovectores) derivados del EOF y aislar las señales físicas del ruido estocástico, se implementa el método de remuestreo no paramétrico Bootstrap-t \citep{efron1993introduction}. La elección de la variante Bootstrap-t sobre el Bootstrap percentílico estándar responde a que, como establece \citet{hesterberg2015bootstrap}, esta variante corrige dinámicamente la asimetría de cada submuestra al estandarizar el estadístico por su propio error estándar, produciendo intervalos de confianza con cobertura más precisa para las distribuciones asimétricas y de cola pesada propias de la precipitación y el viento ciclónico. Se generan $R = 15{,}000$ submuestras con reemplazo, garantizando que el error de Monte Carlo sea insignificante en la estimación de las colas de la distribución.

% --- Estadístico y criterio de significancia ---
...
% --- PC1 como indicador de la proyección de cada ciclón sobre el modo dominante ---
...
\subsubsection{Relación entre PC1 e intensidad dinámica del ciclón}\label{subsubsec:pc1_vorticidad}

% --- Correlación PC1 vs. ζ850_min ---
Para establecer la conexión física entre la estructura de los campos de superficie y la intensidad sinóptica del sistema, se evalúa la correlación de Pearson entre la PC1 de cada variable y el mínimo de vorticidad relativa alcanzado por cada ciclón, $\zeta_{850}^{\min}$:
...
\subsubsection{Coherencia entre modos dominantes de precipitación y viento}\label{subsubsec:coherencia_pc1}

% --- Correlación entre PC1 de precipitación y PC1 de viento ---
Para evaluar si los patrones dominantes de ambas variables varían conjuntamente, se calcula la correlación entre las PC1 de precipitación y viento dentro de cada región y fase:

\begin{equation}
    \rho = \text{corr}\!\left(PC_1^{\text{precip}},\; PC_1^{\text{viento}}\right).
\end{equation}

Valores cercanos a $+1$ indican que los extremos de precipitación y viento se organizan espacialmente de manera coherente (eventos compuestos bien estructurados); valores cercanos a $0$ sugieren que la variabilidad espacial de la precipitación está controlada por procesos locales independientes de la configuración del campo de viento a escala sinóptica.

% ============================================================
%  SECCIÓN M5 — ANÁLISIS MULTIVARIADO Y PROTOTIPOS
%  Propósito: Describir el MEOF como herramienta de análisis
%  conjunto de precipitación y viento, y el procedimiento de
%  construcción de los prototipos estructurales. Incluye:
%  (M5a) normalización y concatenación del vector de estado,
%  (M5b) composito condicionado al MEOF1 (selección de
%        ciclones con PC1 > umbral p80 y PC1 > 0),
%  (M5c) superposición de capas estadísticas en la figura
%        matricial 3×4.
% ============================================================
...
% --- Justificación y procedimiento de normalización ---
Para evaluar la covariabilidad espacial entre los campos de precipitación y viento, se realiza un análisis EOF combinado (MEOF). Dado que estas variables poseen unidades físicas y rangos de varianza dispares (mm\,h$^{-1}$ vs.\ m\,s$^{-1}$), es un requisito metodológico estandarizarlas antes de su integración para evitar que la variable con mayor magnitud numérica domine los modos resultantes \citep{wilks2019statistical_ch13}. El procedimiento consta de tres pasos:
...
\paragraph{Paso 3 — Concatenación espacial.} Los campos normalizados se concatenan para formar el vector de estado combinado, siguiendo el esquema de análisis de patrones acoplados de \citet{bretherton1992intercomparison}:
 
\begin{equation}\label{eq:meof_vector}
    \mathbf{Y}(\tau) = \big[ Z_P(\tau),\; Z_W(\tau) \big].
\end{equation}
 
El análisis EOF aplicado a la matriz combinada $\mathbf{Y}(\tau)$ produce modos acoplados que explican la varianza conjunta del sistema, siguiendo la base matemática validada por \citet{liang2018multivariate}. El MEOF$_1$ produce simultáneamente un patron espacial para precipitación ($\text{EOF}_{1,P}$) y otro para viento ($\text{EOF}_{1,W}$), ambos extraídos del mismo modo dominante de covariabilidad conjunta.
 
\subsection{Construcción de los prototipos estructurales}\label{subsec:prototipos}
 
% --- Descripción de las cuatro capas estadísticas ---
...
\paragraph{Capas 1 y 2 — Isolíneas PDFe de ubicación de máximos (precipitación y viento).}
A partir de la superficie de densidad $\hat{f}(\mathbf{s})$ calculada según la Ecuación~\ref{eq:kde_simple}, se extraen los contornos correspondientes a los percentiles 75 y 90 de la densidad acumulada (Ecuación~\ref{eq:nivel_contorno_kde}). Sobre cada superficie se señaliza mediante un marcador estelar la coordenada modal $(\Delta x_{\text{moda}}, \Delta y_{\text{moda}})$ definida por la Ecuación~\ref{eq:moda_kde}, indicando la posición donde estadísticamente es más frecuente encontrar el máximo de la variable.
 
\paragraph{Capas 3 y 4 — Isolíneas de los patrones espaciales del MEOF$_1$ (precipitación y viento).}
Se superponen las isolíneas de $\text{EOF}_{1,P}$ y $\text{EOF}_{1,W}$, obtenidas del primer modo del análisis multivariado descrito en la Sección~\ref{subsec:eof_multivariado}. Previo al trazado de contornos, se aplica un suavizado gaussiano ($\sigma = 2$) a cada campo. Los niveles de contorno se determinan mediante percentiles aplicados separadamente sobre los valores positivos (percentiles 33 y 66, representados con línea sólida) y negativos (percentiles 33 y 66 del valor absoluto, línea discontinua). Esta estrategia de umbralización relativa garantiza comparabilidad entre distintas combinaciones región--fase, independientemente de la magnitud absoluta del patron espacial del MEOF. Las regiones de carga positiva indican sectores donde la variable tiende a presentar anomalías positivas respecto a la media cuando el modo dominante se activa; las de carga negativa, supresión relativa.
 
% --- Figura matricial resultante ---
\subsubsection{Figura matricial 3 $\times$ 4}\label{subsubsec:figura_matricial}
 
El resultado es una figura matricial de $3 \times 4$ paneles (3 regiones de ciclogénesis $\times$ 4 fases evolutivas) donde cada panel integra las cuatro capas descritas: PDFe de precipitación (contornos en azul), PDFe de viento (contornos en rojo), $\text{EOF}_{1,P}$ de precipitación (isolíneas en verde) y $\text{EOF}_{1,W}$ de viento (isolíneas en naranja). La superposición permite identificar el grado de acoplamiento espacial entre la variabilidad estructural dominante (MEOF) y la distribución de probabilidad de los máximos (PDFe): una alta coincidencia entre el núcleo del PDFe y la región de carga positiva del EOF indica que el patrón dominante de variabilidad está directamente asociado a la ocurrencia de los valores extremos; un desacoplamiento señala procesos locales o asimetrías frontales que organizan los impactos en sectores distintos al patrón modal. La coordenada modal de cada variable, anotada en la leyenda interna de cada panel como $(\Delta x, \Delta y)$ en grados, cuantifica explícitamente el desplazamiento preferencial de los máximos respecto al centro del vórtice. Este procedimiento se ejecuta de forma independiente para la Población Global y el subconjunto p90, permitiendo la comparación directa de los patrones estructurales entre ambos estratos de intensidad.