...
\begin{equation}\label{eq:pc1_correlacion_zeta}
    \rho\!\left(PC_1, \zeta_{850}^{\min}\right) = \frac{\text{cov}\!\left(PC_1, \zeta_{850}^{\min}\right)}{\sigma_{PC_1} \cdot \sigma_{\zeta}}.
\end{equation}
Su interpretación físico-estadística, el vínculo entre la proyección sobre el modo dominante y la intensidad dinámica del sistema, se estableció en la Sección~\ref{subsec:tb_eof} de los Fundamentos.
...
\begin{equation}\label{eq:pc1_correlacion_precip_viento}
    \rho = \text{corr}\!\left(PC_1^{\text{precip}},\; PC_1^{\text{viento}}\right).
\end{equation}
Valores cercanos a $+1$ indican que precipitación y viento se organizan espacialmente de manera coherente; valores cercanos a $0$ sugieren que la variabilidad espacial de la precipitación está controlada por procesos locales independientes de la configuración de mayor escala; valores marcadamente negativos indicarían una relación inversa entre ambos campos, sin respaldo físico claro dentro del marco de acoplamiento WCB--CCB descrito en la Sección~\ref{subsec:tb_compound_def}, y ameritarían una revisión adicional del caso.
