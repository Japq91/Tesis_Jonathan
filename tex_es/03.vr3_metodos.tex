...
La resolución espacial ($\sim$31 km) y temporal (horaria) de ERA5, junto con su mayor densidad de trayectorias y representación de estructuras de mesoescala frente a reanálisis previos \citep{gramcianinov2020analysis}, justifican su elección para el catálogo de trayectorias.
...
La elección de ambas variables responde a la naturaleza morfológica de los ciclones extratropicales y a la estructura de sus campos de precipitación y viento, fundamentada en la Sección~\ref{sec:tb_compound_def} de los Fundamentos.
...
\section{Poblaciones de estudio}\label{sec:poblaciones_estudio}

La base de trayectorias se divide en una población de referencia y, a partir de ella, un subconjunto de alta intensidad ($p90$), como se detalla en la Sección~\ref{subsec:tb_poblaciones}
Ambas poblaciones se construyen de forma independiente para cada una de las tres regiones de ciclogénesis (SBR, LPB y ARG), delimitadas mediante dominios geográficos fijos basados en la taxonomía de \citet{gramcianinov2019properties} y detallados en la Tabla~\ref{tab:regiones_coords}.
Esta estratificación geográfica responde a que las regiones presentan regímenes dinámicos diferenciados, discutidos en la Sección~\ref{sec:tb_regiones}.
...
Para cada ciclón de la PG se identifica el valor más negativo de vorticidad relativa a 850 hPa, $\zeta_{850}^{\min}$, alcanzado en algún momento de su vida, como métrica de intensidad para la extracción posterior del subconjunto p90.
...
Para evaluar los patrones espaciales frente a la severidad dinámica del sistema, se extrae de la PG, de forma independiente para cada región de ciclogénesis, el 10\% de ciclones con los valores más negativos de $\zeta_{850}^{\min}$ (un decil de la distribución), obteniendo el subconjunto $p90$ (detallado en \ref{subsec:tb_poblaciones}).
...
Para caracterizar la estructura de los sistemas independientemente de su ubicación geográfica, se adopta el marco lagrangiano expuesto en la Sección~\ref{subsec:tb_lagrangiano} de los Fundamentos, un dominio móvil de $20^{\circ} \times 20^{\circ}$ centrado en la posición del mínimo de vorticidad para cada instante $t$ de la trayectoria, en coordenadas relativas $(\Delta x, \Delta y)$ respecto al núcleo ciclónico. Este dominio aísla la señal de mesoescala y representa físicamente la extensión espacial de los brazos frontales del sistema, dimensión también usada por \citet{cardoso2022synoptic} en el Atlántico Sur. El procesamiento se realiza por separado para cada combinación de región de ciclogénesis y fase del ciclo de vida.

Siguiendo la técnica de compositing centrado en el sistema (Sección~\ref{subsec:tb_compositing}), se aplica un promediado temporal centrado para reducir la variabilidad de alta frecuencia de los datos horarios y obtener el campo más representativo de cada fase. Para cada ciclón y fase, se promedian únicamente los registros horarios más próximos al punto medio temporal de esa fase: dos registros cuando su duración total es par (los dos pasos centrales, equidistantes del punto medio), y tres cuando es impar (el paso central exacto más uno adyacente a cada lado). Este criterio evita que el promedio mezcle dos regímenes evolutivos distintos, de modo que el campo resultante representa la condición arquetípica de la fase (incipiente, intensificación, madurez o decaimiento) y no una transición entre estados sucesivos. El resultado es un único campo promedio por combinación ciclón--fase, unidad de análisis de todas las etapas estadísticas subsiguientes. En consecuencia, cada ciclón aporta exactamente un campo a cada una de las cuatro poblaciones fase-específicas de su región, por lo que el tamaño muestral $m$ es constante entre fases dentro de una misma región e igual al número de ciclones reportado en la Tabla~\ref{tab:conteo_ciclones}.
...
Para identificar los patrones espaciales dominantes de variabilidad estructural se aplica un análisis EOF sobre el conjunto de $m$ campos promedio por ciclón--fase, por separado para cada variable (precipitación, viento), región y fase. Las referencias previas de este enfoque se presentaron en la Sección~\ref{subsec:tb_eof}. Previo al análisis, a cada campo se le resta la media del ensamble en cada punto de grilla para obtener las anomalías estructurales:

\begin{equation}\label{eq:eof_anomalia}
    X'(\tau, \mathbf{s}) = X(\tau, \mathbf{s}) - \frac{1}{m}\sum_{\tau'=1}^{m}X(\tau', \mathbf{s}),
\end{equation}

donde $X(\tau, \mathbf{s})$ es el campo de la variable para el ciclón $\tau$ en la posición relativa $\mathbf{s} = (x,y)$.

El análisis EOF univariado se aplica en dos instancias, con un tratamiento ligeramente distinto de esta anomalía según el propósito.

\paragraph{Modos dominantes y varianza explicada.} Para las figuras de EOF1 y de fraccionamiento de varianza (Secciones~\ref{subsec:eof1_wind10_global}--\ref{subsec:eof1_wind10_p90}, \ref{subsec:eof1_tp_global}--\ref{subsec:eof1_tp_p90}, \ref{sec:var_exp_wind10} y \ref{subsec:var_exp_tp}), la anomalía se normaliza por un escalar global, con la misma lógica aplicada al MEOF (Sección~\ref{subsec:eof_multivariado}, Ecuación~\ref{eq:meof_normalizacion}):
...
donde $\sigma_X(\mathbf{s}_i)$ es la desviación estándar entre ciclones en el punto de grilla $\mathbf{s}_i$. Este escalar único preserva la forma espacial de la anomalía mientras normaliza su magnitud, y se calcula de forma independiente para cada variable, región y fase mediante \texttt{scikit-learn}. Como consecuencia, las cargas de EOF$_k$ mostradas en los mapas son adimensionales (en unidades de $\bar\sigma_X$), no la magnitud física original de la variable.

\paragraph{Significancia estadística.} Para el test Bootstrap-t (Sección~\ref{subsec:bootstrap_significancia}), el EOF se calcula directamente sobre la anomalía $X'(\tau,\mathbf{s})$ sin este paso de normalización, mediante la biblioteca \texttt{xeofs} \citep{rieger2024xeofs} sobre estructuras \texttt{xarray}. Dado que ambos procedimientos difieren únicamente por un factor de escala constante, igual para todo el dominio y todos los ciclones de un grupo, el porcentaje de varianza explicada por cada modo es idéntico entre ambos; solo cambia la unidad de las cargas espaciales, que en este caso conservan la magnitud física original de la variable.

En ambos casos, la reconstrucción del campo a partir de los modos ortogonales sigue la misma expresión general:

\begin{equation}\label{eq:eof_reconstruccion}
    X'(\tau, \mathbf{s}) = \sum_{k=1}^{K} \text{PC}_k(\tau) \cdot \text{EOF}_k(\mathbf{s}).
\end{equation}

No se aplica remoción de tendencia temporal (\textit{detrend}). El eje muestral es un índice de eventos discretos con espaciado temporal irregular entre 1979--2020, por lo que un ajuste lineal cronológico asumiría erróneamente un espaciado constante. El objetivo físico es aislar las características morfológicas de la variabilidad inter-sistémica dentro de cada fase, no las tendencias climáticas de baja frecuencia.
...
La interpretación física de cada modo --como flujo sinóptico de fondo o como circulación de mesoescala-- se apoya en la escala espacial del patrón, su ubicación relativa al centro del ciclón y su comparación cualitativa con la literatura; no se contrasta aquí contra viento vectorial, diagnósticos frontales independientes o índices de circulación de gran escala. Estas atribuciones se leen, por tanto, como lecturas físicamente plausibles y no como identificaciones confirmadas.

\subsection{Significancia espacial: Bootstrap-t}\label{subsec:bootstrap_significancia}

Para evaluar la robustez estadística de los autovectores del EOF y aislar las señales físicas del ruido estocástico, se implementa el remuestreo no paramétrico Bootstrap-t; su elección y ventajas frente al Bootstrap percentílico estándar para distribuciones asimétricas y de cola pesada se fundamentaron en la Sección~\ref{subsec:tb_bootstrap} de los Fundamentos. Se generan $R = 15{,}000$ submuestras con reemplazo, garantizando error de Monte Carlo insignificante en la estimación de las colas de la distribución.

Para cada iteración $b$ se recalcula el EOF sobre la submuestra y, en cada punto de grilla, se calcula el estadístico:

\begin{equation}\label{eq:bootstrap_t_espacial}
    t^*_b(\mathbf{s}) = \frac{\hat{\theta}^*_b(\mathbf{s}) - \hat{\theta}(\mathbf{s})}{SE^*_b(\mathbf{s})},
\end{equation}

donde $\hat{\theta}(\mathbf{s})$ es la carga espacial original en $\mathbf{s}$, $\hat{\theta}^*_b(\mathbf{s})$ su estimación en la submuestra, $SE^*_b(\mathbf{s})$ el error estándar de dicha submuestra, y $SE(\mathbf{s})$ el error estándar de la carga original (calculado sobre la muestra completa, sin remuestrear). Los intervalos de confianza al 95\% se obtienen invirtiendo los cuantiles empíricos $q_{\alpha/2}$ y $q_{1-\alpha/2}$ de la distribución de $t^*$ (con $\alpha = 0.05$):
...
Un punto de grilla se considera estadísticamente significativo cuando su intervalo de confianza excluye el cero, lo que equivale a rechazar, al nivel $\alpha = 0.05$ ya definido, la hipótesis nula de que la carga espacial en ese punto no difiere de cero por azar de muestreo.

\subsection{Análisis de las Componentes Principales (PC1--PC3)}\label{subsec:analisis_pc}

Además de la descomposición espacial, se usará la Componente Principal del primer modo (PC1), que representa la amplitud de cada ciclón sobre el patrón estructural dominante; la interpretación de estos estadísticos se estableció en la Sección~\ref{subsec:tb_eof} de los Fundamentos. 
Se calculan, para la serie de PC1 de cada variable, región y fase, la asimetría ($\gamma_1$):
...
Para conectar físicamente la estructura de los campos de superficie con la intensidad del sistema, se evalúa la correlación de Spearman entre la PC1, la PC2 y la PC3 de cada variable, y la vorticidad relativa a 850 hPa asociada. 
Esta última se obtiene de los mismos instantes centrales usados para construir los campos compuestos (Sección~\ref{sec:procesamiento_lagrangiano}), tomando el de mayor magnitud entre ellos como representativo de la intensidad del sistema en esa fase; no debe confundirse con el $\zeta_{850}^{\min}$ de toda la vida del ciclón empleado en la Sección~\ref{subsec:grupo_global} para la selección del subconjunto p90.

El signo de la PC indica de forma reproducible a qué lado del patrón dominante cae cada ciclón. 
Correlacionar la PC con signo contra una variable externa prueba entonces si ese lado del patrón está asociado a la intensidad del sistema, más allá de qué tan marcada esté la proyección. 
Se emplea a Spearman para no asumir normalidad en la distribución de las PC. 
Su interpretación físico-estadística, el vínculo entre el lado del patrón dominante en que cae un ciclón y su intensidad dinámica, se estableció en la Sección~\ref{subsec:tb_eof} de los Fundamentos.

Para evaluar si los patrones dominantes de precipitación y viento varían conjuntamente, se calcula la correlación de Spearman entre la PC1 de precipitación y la PC1 del viento, dentro de cada región y fase. 
Valores cercanos a $+1$ o a $-1$ indican que el lado del patrón dominante de un campo está sistemáticamente asociado al lado del patrón dominante del otro; valores cercanos a $0$ sugieren que ambos patrones varían de forma independiente entre sí.
...
\paragraph{Paso 2 — Normalización por desviación estándar espacialmente promediada.} Para preservar los gradientes espaciales nativos de cada campo, la normalización usa un escalar global por variable, en lugar de la estandarización punto a punto de la PCA sobre matriz de correlación:
...
\end{equation}
donde $V'$ es el campo de anomalías, $\sigma_{V}(\mathbf{s}_i)$ la desviación estándar entre eventos en el punto de grilla $\mathbf{s}_i$, y $N_s$ el número de puntos de grilla del dominio. Este escalar preserva los gradientes espaciales mientras equilibra el peso de ambas variables en la matriz de covarianza, evitando que el campo de mayor varianza domine la descomposición conjunta, problema documentado para variables de unidades dispares en \citet{wilks2019statistical_ch13}.
...
La superposición permite leer el grado de organización espacial entre ambos campos en el entorno del ciclón: una alta coincidencia entre el núcleo del PDFe de precipitación y la región de carga positiva del EOF de viento indica una estructura frontal compacta; un desacoplamiento revela asimetrías en la distribución de los máximos.
Este análisis caracteriza la morfología de la fase y, de forma secundaria, ofrece un marco de referencia para identificar configuraciones que favorecerían eventos simultáneos de viento y precipitación.
