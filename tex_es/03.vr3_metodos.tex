...
Para caracterizar la estructura de los sistemas independientemente de su ubicación geográfica, se adopta el marco lagrangiano expuesto en la Sección~\ref{subsec:tb_lagrangiano} de los Fundamentos, un dominio móvil de $20^{\circ} \times 20^{\circ}$ centrado en la posición del mínimo de vorticidad para cada instante $t$ de la trayectoria, en coordenadas relativas $(\Delta x, \Delta y)$ respecto al núcleo ciclónico. Este dominio aísla la señal de mesoescala y representa físicamente la extensión espacial de los brazos frontales del sistema, dimensiones tambien usadas por \citet{pepler2020dimensional, cardoso2022synoptic}. El procesamiento se realiza por separado para cada combinación de región de ciclogénesis y fase del ciclo de vida.
...
\end{equation}
Para conectar físicamente la estructura de los campos de superficie con la intensidad del sistema, se evalúa la correlación de Spearman entre el valor absoluto de la PC1 de cada variable, $|PC_1|$, y la vorticidad relativa a 850 hPa asociada a la fase. Esta última se obtiene de los mismos instantes centrales usados para construir los campos compuestos (Sección~\ref{sec:procesamiento_lagrangiano}), tomando, en lugar del promedio, el de mayor magnitud entre ellos como representativo de la intensidad del sistema en esa fase; no debe confundirse con el $\zeta_{850}^{\min}$ de toda la vida del ciclón empleado en la Sección~\ref{subsec:grupo_global} para la selección del subconjunto p90. Se utiliza el valor absoluto en ambas variables porque el signo de la PC1 depende de una convención arbitraria del análisis EOF (Sección~\ref{subsec:tb_eof} de los Fundamentos), y porque la vorticidad relativa se registra en la base de datos en magnitud, sin signo; el interés aquí es la fuerza de la proyección sobre el modo dominante, no su fase. Se emplea un coeficiente de correlación de rangos, no paramétrico, por no asumirse normalidad en la distribución de $|PC_1|$. Su interpretación físico-estadística, el vínculo entre la fuerza de la proyección sobre el modo dominante y la intensidad dinámica del sistema, se estableció en la Sección~\ref{subsec:tb_eof} de los Fundamentos.

Para evaluar si la fuerza de los patrones dominantes de precipitación y viento varía conjuntamente, se calcula la correlación de Spearman entre $|PC_1^{\text{precip}}|$ y $|PC_1^{\text{viento}}|$ dentro de cada región y fase. Valores cercanos a $+1$ indican que, cuando el patrón dominante de un campo está fuertemente marcado en un evento, el del otro campo también lo está; valores cercanos a $0$ sugieren que la fuerza de ambos modos varía de forma independiente; valores marcadamente negativos indicarían que la fuerza de un modo tiende a ser alta cuando la del otro es baja, relación que ameritaría una revisión adicional del caso dentro del marco de acoplamiento WCB--CCB descrito en la Sección~\ref{subsec:tb_compound_def}.
